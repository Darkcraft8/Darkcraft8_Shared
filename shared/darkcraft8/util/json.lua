...
-- Local Sub Functions --
    local function varIsTableOrArray(_, var, jsonArray)
    ...
    local function varExist(_, var, jsonArray)
        if (jsonArray or {})[var] then return true else return false end
    end

    local function luaToPatch_Analyse(_, new, old, parent)
        local patch = {}
        local path = "/"
        if parent then
            path = parent .. "/"
        ...
        for var, val in pairs(new) do
            ...
            if varExist(var, old) then
                ...
                elseif type == "array" then
                    ...
                    for index, patchSection in ipairs(luaToPatch_Analyse(val, old[var], path .. var)) do 
                        table.insert(patch, patchSection)
                    end
...
function json.luaToPatch(old, new) -- create and print in the log a json patch that patch the first array into the second
    ...
    local newPatch = luaToPatch_Analyse(new, old)
